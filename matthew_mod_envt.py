...
class MatthewEnvt:
	...
	def reset(self):
		agent_types = [0,0,0,1,1,1,2,2,2,3]
		agent_types = [0,0,0,0,0,0,2,2,2,2]
		# agent_types = [300, 100,100,100,10,10,10,0,0,0]
		#shuffle agent types
		# self.agent_types = np.random.permutation(agent_types)
		...
		su = [0]*self.n_agents
		targets = []
		size_range = (self.max_size-self.min_size)/500
	...
	def set_speed_from_sizes(self):
		for i in range(self.n_agents):
			...
			# self.speed[i] = self.base_speed + self.size[i]*(self.agent_types[i]+1)
