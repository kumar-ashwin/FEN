...
while i_episode<n_episode:
	...
	while steps<max_steps:
		...
		if steps%T==0 and training:
			...
			for experience in experiences:
				...
				opt_actions = compute_best_actions(VF, loc_obs, M_train.targets, n_agents, n_resources, M_train.su, beta=0.0, epsilon=0)
	...
	if i_episode%1000==0:
		VF.save_model(f"Models/DQN/{mode}/{int(st_time)}/model_{i_episode}.ckpt")
	...
	if i_episode%200==0 and training:
		...
		if np.mean(val_utility)>best_val_utility:
			...
			os.makedirs(f"Models/DQN/{mode}/{int(st_time)}/best", exist_ok=True)
			VF.save_model(f"Models/DQN/{mode}/{int(st_time)}/best/best_model.ckpt")
			print("Saved best model")
			#Write the logs to a file
			with open(f"Models/DQN/{mode}/{int(st_time)}/best/best_log.txt", "w") as f:
