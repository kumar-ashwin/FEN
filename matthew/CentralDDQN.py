...
Central agent implementation: Select from joint actions
"""
import os, sys, time  
import numpy as np
...
from matching import compute_best_joint_action, SI_reward, variance_penalty, compute_best_joint_action
...
if training and logging:
	...
	log_dir = f"logs/CentralDDQN/{mode}/{int(st_time)}/"
	print("Logging to {} \n\n\n\n".format(log_dir))
...
obs = M.get_central_obs()
num_features = len(obs[0])
num_agent_features = len(obs[1][0])

# model_loc = "Models/CentralDDQN/FixedComplex/0/1689619731/best/best_model.ckpt"
...
def simple_score(state):
	#larger is worse
	#last feature of every agent is the distance to the nearest resource
	dists = []
	for i in range(n_agents):
		dists.append(state[num_agent_features*i+num_agent_features-1])
	#Sum the squares of the distances
	distance = sum([d**2 for d in dists]) - 100*100*7
	return distance
...
while i_episode<n_episode:
	...
	while steps<max_steps:
		...
		if selected_VF_id==0:
			actions = compute_best_joint_action(VF1, M, obs, M.targets, n_agents, n_resources, M.discounted_su, beta=beta, epsilon=ep_epsilon)
		else:
			actions = compute_best_joint_action(VF2, M, obs, M.targets, n_agents, n_resources, M.discounted_su, beta=beta, epsilon=ep_epsilon)
		...
		# print(actions)
		#num of targets non zero
		occupied_resources = set([M.targets[j][0] for j in range(n_agents) if M.targets[j] is not None])
		# print(sum([1 for a in actions if a!=-1])+ len(occupied_resources), end='\r')
		pd_states = M.get_post_decision_central_state(obs, actions)
		...
		experience = copy.deepcopy([pd_states, sum(rewards), M.get_state()])
		...
		if steps%T==0 and training:
			...
			for experience in experiences:
				#Compute best action
				old_pd_state, old_reward, new_state = experience
				M_train.set_state(new_state)
				succ_obs = M_train.get_central_obs()
				opt_actions = compute_best_joint_action(TargetNetwork1, M, succ_obs, M_train.targets, n_agents, n_resources, M_train.discounted_su, beta=beta, epsilon=0.0)
				new_pd_state = M_train.get_post_decision_central_state(succ_obs, opt_actions)

				# if central_rewards:
				# 	old_reward = [np.mean(old_reward)]*n_agents
				
				# # old_reward = np.array(old_reward)
				# if learning_beta>0:
				...
				old_pd_states.append(old_pd_state)
				old_rewards.append(old_reward)
				new_pd_states.append(new_pd_state)
			
			target_values = np.array([old_rewards[i] + GAMMA * TargetNetwork2.get([new_pd_states[i]]) for i in range(len(new_pd_states))]).reshape(-1)

			# print(target_values)
			#Perform batched updates

			states = np.array(old_pd_states)
			...
			for experience in experiences2:
				...
				M_train.set_state(new_state)
				succ_obs = M_train.get_central_obs()
				opt_actions = compute_best_joint_action(TargetNetwork2, M_train, succ_obs, M_train.targets, n_agents, n_resources, M_train.discounted_su, beta=beta, epsilon=0.0)
				new_pd_state = M_train.get_post_decision_central_state(succ_obs, opt_actions)
			...
			target_values = np.array([old_rewards[i] + GAMMA * TargetNetwork1.get([new_pd_states[i]]) for i in range(len(new_pd_states))]).reshape(-1)
	...
	if i_episode%1000==0:
		VF1.save_model(f"Models/CentralDDQN/{mode}/{int(st_time)}/model_{i_episode}.ckpt")

	# # Validation runs every 500 episodes, to select best model
	if i_episode%20==0 and training:
		print("Validating")
		mult = 1
		update = False
		if i_episode%100==0:
		...
		for val_eps in range(1*mult):
			print(val_eps, end='\r')
			M_val.reset()
			obs = M_val.get_central_obs()
			score = 0
			for steps in range(max_steps):
				actions = compute_best_joint_action(VF1, M_val, obs, M_val.targets, n_agents, n_resources, M_val.discounted_su, beta=beta, epsilon=0)
		...
		if  update and np.mean(val_utility)>best_val_utility:
			...
			os.makedirs(f"Models/CentralDDQN/{mode}/{int(st_time)}/best", exist_ok=True)
			VF1.save_model(f"Models/CentralDDQN/{mode}/{int(st_time)}/best/best_model.ckpt")
			print("Saved best model")
			#Write the logs to a file
			with open(f"Models/CentralDDQN/{mode}/{int(st_time)}/best/best_log.txt", "w") as f:
