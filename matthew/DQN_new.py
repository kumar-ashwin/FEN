...
if training and logging:
	...
	log_dir = f"logs/DQN_new/{mode}/{int(st_time)}/"
	print("Logging to {} \n\n\n\n".format(log_dir))
...
VF = ValueNetwork(num_features=num_features, hidden_size=256, learning_rate=0.00003) #TODO: Add num other agents
TargetNetwork = ValueNetwork(num_features=num_features, hidden_size=256, learning_rate=0.00003)
...
while i_episode<n_episode:
	...
	while steps<max_steps:
		...
		if steps%T==0 and training:
			...
			for experience in experiences:
				...
				opt_actions = compute_best_actions(VF, succ_obs, M_train.targets, n_agents, n_resources, M_train.su, beta=beta, epsilon=0.0)
				...
				target_values = np.array([old_rewards[i] + GAMMA * TargetNetwork.get(np.array([new_pd_actions[i]])) for i in range(n_agents)])
	...
	if i_episode%1000==0:
		VF.save_model(f"Models/DQN_new/{mode}/{int(st_time)}/model_{i_episode}.ckpt")
	...
	if i_episode%10==0 and training:
		...
		for val_eps in range(1*mult):
			...
			for steps in range(max_steps):
				actions = compute_best_actions(VF, obs, M.targets, n_agents, n_resources, M.su, beta=beta, epsilon=0)
		...
		if update and np.mean(val_utility)>best_val_utility:
			...
			os.makedirs(f"Models/DQN_new/{mode}/{int(st_time)}/best", exist_ok=True)
			VF.save_model(f"Models/DQN_new/{mode}/{int(st_time)}/best/best_model.ckpt")
			print("Saved best model")
			#Write the logs to a file
			with open(f"Models/DQN_new/{mode}/{int(st_time)}/best/best_log.txt", "w") as f:
