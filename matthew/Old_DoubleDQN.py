...
fairness_type = "split_variance" #Should exist
fairness_type = "SI"
...
while i_episode<n_episode:
	...
	while steps<max_steps:
		...
		experience = copy.deepcopy([pd_states, rewards, M.get_state(), f_rewards])
		...
		if steps%T==0 and training:
			...
			for experience in experiences:
				#Compute best action
				old_pd_states, old_rewards, new_state, old_f_rew = experience
				...
				if learning_beta>0:
					...
					# print(fair_rewards)
					old_rewards = old_rewards + learning_beta*np.array(old_f_rew)
			...
			for experience in experiences2:
				...
				old_pd_states, old_rewards, new_state, old_f_rew = experience
				M_train.set_state(new_state)
				su_prev = copy.deepcopy(M_train.discounted_su)
				succ_obs = M_train.get_obs()
