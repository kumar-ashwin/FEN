...
while i_episode<n_episode:
	...
	if i_episode%200==0 and training:
		...
		if np.mean(val_utility)>best_val_utility:
			...
			mode = "Reallocate" if reallocate else "Fixed"
			mode += "Central" if central_rewards else ""
			os.makedirs(f"Models_matthew/OnPolicyVF/{mode}/{int(st_time)}/best", exist_ok=True)
			VF.save_model(f"Models_matthew/OnPolicyVF/{mode}/{int(st_time)}/best/best_model.ckpt")
			print("Saved best model")
			#Write the logs to a file
			with open(f"Models_matthew/OnPolicyVF/{mode}/{int(st_time)}/best/best_log.txt", "w") as f:
